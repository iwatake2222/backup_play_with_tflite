...
#define MODEL_FILENAME_BASE RESOURCE"/deeplabv3_mnv2_dm05_pascal_quant"
...
int main()
{
	...
	int frameCnt = 0;
	/* Define logics */
	auto requestProducer = [&interpreters, &runner, &frameCnt]() {
		...
		while(1) {
			/* To avoid pushing too much requests, skip frame if theare are some requests which are not consumed. */
			auto stats = runner->GetSegmentStats();
			int consumedRequestNum = stats[0].num_inferences;
			if (consumedRequestNum < frameCnt) continue;
			...
			for (int i = 0; i < MODEL_SEGMENTS_NUM; i++) {
				printf("   %d: %.3lf [msec]\n", i, stats[i].total_time_ns / 1000000.0 / stats[i].num_inferences);
			}
		}
	...
	auto requestConsumer = [&interpreters, &runner]() {
		...
		while (runner->Pop(&response)) {
			/* Retrieve the result */
			const int64_t *outputMap = response[0].data.i64;

			/* Display result */
			cv::Mat outputImage = cv::Mat::zeros(MODEL_HEIGHT, MODEL_WIDTH, CV_8UC3);
			for (int y = 0; y < MODEL_HEIGHT; y++) {
				for (int x = 0; x < MODEL_WIDTH; x++) {
					if (outputMap[y * MODEL_WIDTH + x] != 0) {
						const int CHANNE_NUM = 3;
						outputImage.data[(y * MODEL_WIDTH + x) * CHANNE_NUM] = 0xFF;
					}
				}
			}
	...
	printf("Total time = %f [msec]\n", timeSpan.count() * 1000.0 / frameCnt);
